// From https://github.com/jay-hodgson/markdown-it-inline-comments/blob/master/index.js
...
function inlinecomment(state, silent) {
  ...
  while (state.pos + 3 <= max) {
    if (
      state.src.charCodeAt(state.pos) === 45 /* - */ &&
...
export function inline_comment_plugin(md) {
